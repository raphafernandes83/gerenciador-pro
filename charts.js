...
// Helper function para valida√ß√£o de chartInstance
function isValidChartInstance(chartInstance, functionName = 'charts') {
    if (!chartInstance) {
        logger.debug(`üî ${functionName}: chartInstance n√£o fornecida`);
        return false;
    }

    if (!chartInstance.data || !chartInstance.data.datasets || !chartInstance.data.datasets[0]) {
        logger.debug(`üî ${functionName}: chartInstance mal configurada`);
        return false;
    }
...
// ?? CHECKPOINT 2.2c: Helper de transiÁ„o para DOMManager (CONSOLIDADO)
...
export const charts = {
    ...
    async init() {
        // Inicializar sistema de otimiza√ß√£o de performance
    ...
    /**
     * Agenda uma atualiza√ß√£o de progresso coalescida com otimiza√ß√µes de performance.
    ...
    scheduleProgressUpdate(history) {
        ...
        this._rafId = raf(() => {
            ...
            // Medir performance da atualiza√ß√£o
    ...
    /**
     * Executa atualiza√ß√£o de progresso com profiling de performance
     */
    ...
    // ===== Integra√ß√£o com metas de Stop Win/Loss (puro + seguro) =====
    _buildGoalsProgressSummarySafe() {
        try {
            // üîß CORRE√á√ÉO: Usar state/config diretamente como no backup funcionando
            ...
            // Usa fun√ß√µes puras se a flag estiver ativa e se existirem globalmente; sen√£o, calcula inline m√nimo
    ...
    _applyLockMode(lock) {
        try {
            ...
            // Fallback: aplicar diretamente no badge caso ui n√£o esteja dispon√vel
            const badge = document.getElementById('progress-soft-lock-badge');
            if (badge) {
                const icon = lock.type === 'STOP_WIN' ? 'üéØ' : '‚öÔ∏';
                const msg =
                    lock.type === 'STOP_WIN'
     ...
     * ?? DESABILITADO: Este mÈtodo n„o deve mais criar gr·ficos
     * O gr·fico È gerenciado exclusivamente pelo progress-card-module.js
     */
    initProgressChart() {
        logger.warn('?? initProgressChart() DESABILITADO - O gr·fico È gerenciado pelo progress-card-module.js');

        // ??? PROTE«√O: Sempre retorna true para n„o quebrar cÛdigo que depende deste mÈtodo
        ...
        // ??? PROTE«√O: Evita reinicializaÁ„o se gr·fico j· existe
        if (this.progressMetasChart) {
            logger.warn('?? Gr·fico j· existe, pulando inicializaÁ„o para evitar duplicaÁ„o');
            return true; // Retorna sucesso pois o gr·fico j· est· pronto
        }

        // ??? ValidaÁ„o robusta de DOM com diagnÛstico
        const canvasElement = dom.progressPieChart;
        if (!canvasElement) {
            logger.error('? Canvas progressPieChart n„o encontrado no DOM');
                    logger.debug('üîé DOM dispon√vel:', {
                        keys: Object.keys(dom).filter((key) => key.includes('progress')),
                    });
                    return false;
                }
        
                // üõ°Ô∏ Valida√ß√£o adicional de contexto Canvas
                try {
                    const context = canvasElement.getContext('2d');
                    if (!context) {
                        logger.error('‚å Falha ao obter contexto 2D do canvas');
                        return false;
                    }
                } catch (contextError) {
                    logger.error('‚å Erro ao validar contexto canvas:', { error: String(contextError) });
                    return false;
                }
        
                try {
                    // Destr√≥i qualquer inst√¢ncia existente neste canvas registrada pelo Chart.js
                    try {
                    ...
                    // üîß CORRE√á√ÉO: Configura√ß√µes otimizadas para funcionalidade real
                    const progressConfig = {
                        ...
                        data: {
                            ...
                            datasets: [
                                {
                                    data: [0, 0], // Inicializa com zeros, ser√° atualizado com dados reais
                        ...
                        options: {
                            ...
                            plugins: {
                                ...
                                tooltip: {
                                    enabled: false, // CORRE«√O: Desabilitado para evitar sobreposiÁ„o de gr·fico fantasma
                                },
                            },
                            animation: {
                                duration: 300, // Anima√ß√£o mais r√°pida para melhor responsividade
                                easing: 'easeInOutQuart',
                            },
                        },
                    };
        
                    // üîß CORRE√á√ÉO: Cria√ß√£o mais robusta da inst√¢ncia
                    this.progressMetasChart = new Chart(canvasElement.getContext('2d'), progressConfig);
        
                    // üîß CORRE√á√ÉO: Inicializa com dados padr√£o seguros
                    ...
                    // A exibiÁ„o de texto central deve ser feita via HTML/CSS sobreposto ou plugin seguro.
        
        
                    // üõ°Ô∏ Valida√ß√£o p√≥s-inicializa√ß√£o mais rigorosa
                    if (!this.progressMetasChart ||
                        typeof this.progressMetasChart.update !== 'function' ||
                        !this.progressMetasChart.data ||
                        !this.progressMetasChart.data.datasets) {
                        logger.error('‚å Gr√°fico criado mas com interface inv√°lida');
                        return false;
                    }
        
                    // üîß CORRE√á√ÉO: For√ßa primeira renderiza√ß√£o
                    try {
                        this.progressMetasChart.update('none');
                    } catch (updateError) {
                        logger.warn('‚öÔ∏ Erro na primeira renderiza√ß√£o:', { error: updateError.message });
                    }
        
                    logger.info('‚úÖ Gr√°fico de progresso inicializado com sucesso');
                    return true;
                } catch (error) {
                    // üî Diagn√≥stico detalhado do erro
                    logger.error('‚å Erro ao inicializar gr√°fico de progresso:', {
                    ...
                    // üõ°Ô∏ Cleanup em caso de falha parcial
                    if (this.progressMetasChart) {
                        try {
                            this.progressMetasChart.destroy();
                            this.progressMetasChart = null;
                        } catch (cleanupError) {
                            logger.warn('‚öÔ∏ Erro durante cleanup', { error: cleanupError.message });
                        }
                    }
    ...
    /**
     * ?? Atualiza o progresso das metas com histÛrico da sess„o
     */
    updateProgressChart(sessionHistory = []) {
        ...
        logger
            ...
            .debug('üîÑ CHARTS: Atualizando progresso com hist√≥rico:', {
                length: sessionHistory?.length || 0,
            });

        if (!Array.isArray(sessionHistory)) {
            logger.withRequest(requestId).warn('‚öÔ∏ Hist√≥rico inv√°lido, usando array vazio');
            sessionHistory = [];
        }
        ...
        try {
            ...
            // Normaliza√ß√£o: aceita hist√≥rico com { isWin:boolean } ou { resultado:'win'|'loss' }
            ...
            // Inicializa gr√°fico se n√£o existir
            if (!this.progressMetasChart) {
                logger.withRequest(requestId).info('üéØ Gr√°fico n√£o existe, inicializando...');
                if (!this.initProgressChart()) {
                    performanceTracker.finishOperation(requestId, 'error', {
                        reason: 'chart_init_failed',
                    });
                    logger.withRequest(requestId).error('‚å Falha ao inicializar gr√°fico');
                    return false;
                }
                // Garante assinatura quando o gr√°fico √© (re)criado
                this._ensureStoreSubscription();
            }

            performanceTracker.addMarker(requestId, 'stats_calculation');

            // Define metas padr√£o se n√£o definidas
            const targets = {
            ...
            // üîß CORRE√á√ÉO: Usa novo sistema de c√°lculos integrado
            const stats = this.calculateProgressStats(normalizedHistory);

            // Disponibiliza √∫ltimas estat√sticas para utilit√°rios/diagn√≥sticos leves
            try {
                this.lastStats = stats;
            } catch { }

            // üîß CORRE√á√ÉO: Calcula dados completos do card se fun√ß√£o dispon√vel
            let cardData = null;
            try {
                if (typeof window.calculateProgressCardData === 'function') {
                    // FASE 3: Busca dados anteriores do cache para compara√ß√£o de trends
                ...
                logger.warn('‚öÔ∏ Erro ao calcular dados completos do card:', { error: error.message });
            }
            ...
            // üîß CORRE√á√ÉO: Usa novo sistema de atualiza√ß√£o se dispon√vel
            if (cardData && cardData.isValid && typeof window.updateProgressCardComplete === 'function') {
                try {
                    ...
                    if (updateSuccess) {
                        logger.debug('‚úÖ Card atualizado via novo sistema');

                        // FASE 3: Armazena dados atuais no cache para futuras compara√ß√µes
                        if (window.progressCardCache) {
                            window.progressCardCache.store(cardData);
                            logger.debug('üíæ Dados armazenados no cache para compara√ß√µes futuras');
                        }
                    } else {
                    ...
                    logger.warn('‚öÔ∏ Erro no novo sistema, usando fallback:', { error: updateError.message });
                    // Fallback para sistema antigo
                    this.updateProgressPieChart(stats, goalsSummary);
                    this.updateProgressStatusNew(stats, targets, goalsSummary);
                }
            ...
            logger.withRequest(requestId).info('‚úÖ CHARTS: Progresso atualizado');
            return true;
        } catch (error) {
            performanceTracker.finishOperation(requestId, 'error', { error: error.message });
            logger
                .withRequest(requestId)
                .error('‚å Erro ao atualizar progresso:', { error: String(error) });
            return false;
        }
    },

    /**
     * üìä Calcula estat√sticas do progresso (VERS√ÉO MELHORADA)
     * Integra com o novo sistema de c√°lculos reais
     */
    calculateProgressStats(sessionHistory) {
        try {
            // Importa fun√ß√£o de c√°lculo real dinamicamente
            if (typeof window.calculateRealStats === 'function') {
                return window.calculateRealStats(sessionHistory);
            }

            // Fallback para c√°lculo local se fun√ß√£o externa n√£o dispon√vel
            return this._calculateProgressStatsLocal(sessionHistory);
        } catch (error) {
            logger.error('‚å Erro ao calcular estat√sticas de progresso:', { error: String(error) });
            return this._calculateProgressStatsLocal(sessionHistory);
        }
    },

    /**
     * üìä C√°lculo local de estat√sticas (fallback)
     * @private
     */
    _calculateProgressStatsLocal(sessionHistory) {
        ...
        // Processamento mais robusto das opera√ß√µes
        for (const operacao of sessionHistory) {
            if (!operacao || typeof operacao !== 'object') {
                continue;
            }

            // Determina resultado da opera√ß√£o
            let isWin = null;
            if (typeof operacao.isWin === 'boolean') {
                ...
                continue; // Pula opera√ß√µes sem resultado claro
            }
    ...
    /**
     * üß™ For√ßa dados de teste no gr√°fico
     */
    testProgressWithData(testData = null) {
        logger.info('üß™ TESTE: Aplicando dados de teste no gr√°fico...');
        ...
        // For√ßa atualiza√ß√£o dos displays ANTES do gr√°fico
        ...
        logger.debug('üß™ Dados de teste aplicados COM FOR√áA:', testStats);
    },

    /**
     * üé® Resolve cores CSS dinamicamente
     */
    getResolvedColors() {
    ...
    /**
     * üìä Atualiza status textual do progresso
     */
    updateProgressStatusNew(stats, targets, goalsSummary = null) {
        logger.debug('üìä Atualizando status do progresso:', { stats, targets });

        // Atualiza√ß√£o com fallback
        const elements = {
        ...
        // Atualiza header "Sess√£o Ativa - X ops"
        try {
            if (dom.progressSessionInfo) {
                ...
                dom.progressSessionInfo.textContent = `${isActive ? 'Sess√£o Ativa' : 'Sess√£o Inativa'} ¬∑ ${count} ops`;
            }
        } catch { }
    },

    /**
     * üéØ Atualiza os cart√µes de status (Win / Loss) com mensagens e classes
     */
    _updateStatusCards(stats, targets, goalsSummary = null) {
        try {
            ...
            logger.warn('‚öÔ∏ _updateStatusCards: falha ao atualizar cart√µes', {
                error: String(error),
            });
        }
    },
    ...
    /**
     * ü•ß Atualiza apenas o gr√°fico de pizza
     */
    updateProgressPieChart(stats, goalsSummary = null) {
        // Verifica√ß√µes defensivas: gr√°fico e canvas precisam estar v√°lidos
        if (!this.progressMetasChart || !this.progressMetasChart.canvas) {
            ...
            try {
                if (dom && dom.progressPieChart) {
                    ...
                    // Canvas indispon√vel no DOM; n√£o atualizar agora
                    return false;
                }
            } catch {
                return false;
        ...
        try {
            ...
            // Anel √∫nico Win/Loss - SIMPLIFICADO
            this.progressMetasChart.data.datasets[0].backgroundColor = [
                colors.primary,  // Verde para vit√≥rias
            ...
            // Texto central √© desenhado pelo plugin; esconder r√≥tulo DOM (garantia)
            try {
            ...
            logger.error('‚å Erro ao atualizar gr√°fico de pizza', { error: String(error) });
            return false;
        }
    },

    /**
     * üìä Atualiza cards de informa√ß√£o (sem barras visuais)
     */
    updateProgressInfoCards(stats, targets) {
        ...
        // ReferÍncias aos elementos percentuais removidas - campos n„o existem mais
        ...
        try {
            ...
            if (summary?.goals) {
                ...
                // Status (lado esquerdo da pr√©via)
            ...
            logger.warn('‚öÔ∏ Erro ao atualizar valores monet√°rios:', error.message);
        }
    },

    /**
     * üìä Atualiza barras de progresso com verifica√ß√µes
     */
    updateProgressBarsNew(stats, targets) {
        ...
        try {
            ...
            if (summary?.goals) {
                ...
                // Usa falta com recupera√ß√£o (considera preju√zo atual)
                const faltaRec =
                    typeof g.restanteWinRecoveryAmount === 'number'
                        ? g.restanteWinRecoveryAmount
                        : g.restanteWinAmount;
                // Layout original: mostrar restante at√© a meta
                ...
                // Limite (R$) no preview √© positivo (sem sinal), mantendo coer√™ncia
                ...
                if (dom.lossSessionResult) {
                    ...
                    // cor por sinal (apenas classe; CSS j√° governa as cores globais)
                ...
                // regra simples: se restante == 0 e lucro > 0, destacar positivo no P/L j√° cobre o caso.
                if (dom.winRemainingAmount) {
                    domHelper.remove(dom.winRemainingAmount, 'positive', 'negative'); // ??
                    const restante = Number(faltaRec || 0);
                    // N√£o colorimos restante positivo como negativo para n√£o confundir; mantemos neutro.
        ...
        // Badges de tend√™ncia (n√£o intrusivo; usa prevWinRate/prevLossRate se dispon√veis)
        try {
            ...
            if (wrBadge) {
                if (wrPrev === null || Math.abs(wrDelta) < 0.05) {
                    ...
                    wrBadge.textContent = `${wrDelta > 0 ? '‚ñ≤' : '‚ñº'} ${Math.abs(wrDelta).toFixed(1)} pp`;
            ...
            if (lrBadge) {
                if (lrPrev === null || Math.abs(lrDelta) < 0.05) {
                    ...
                    lrBadge.textContent = `${lrDelta > 0 ? '‚ñ≤' : '‚ñº'} ${Math.abs(lrDelta).toFixed(1)} pp`;
    ...
    /**
     * üìä Atualiza uma barra individual com verifica√ß√µes
     */
    updateProgressBarSafe(type, currentRate, targetRate) {
        logger.debug(`üîÑ Atualizando barra ${type}:`, { currentRate, targetRate });
        ...
        Object.entries(elements).forEach(([key, element]) => {
            if (!element) {
                if (isDevelopment && isDevelopment()) {
                    logger.warn(`‚öÔ∏ Elemento ${type}${key} n√£o encontrado`);
                } else {
                    logger.debug && logger.debug(`Elemento ${type}${key} n√£o encontrado`);
                }
                return;
            }

            try {
                // Normaliza√ß√£o segura para percentuais
                ...
                if (isDevelopment && isDevelopment()) {
                    logger.error(`‚å Erro ao atualizar ${type}.${key}:`, { error: String(error) });
                } else {
        ...
        try {
            ...
            // Cria marcadores se n√£o existirem
            ...
            try {
                ...
                if (type === 'win' && summary?.goals) {
                    ...
                    if (progressV2) {
                        ...
                        currentMarker.title = `Falta (com recupera√ß√£o): ${ui?._formatarMoedaInternal?.(faltaRec) || ''}`;
                    }
                }
            ...
            logger.warn('‚öÔ∏ Falha ao posicionar marcadores de progresso', { error: String(e) });
        }
    },

    updateAssertividadeChart(historico, chartInstance) {
        // ValidaÁ„o defensiva dos par‚metros
        if (!Array.isArray(historico)) {
            logger.warn('?? updateAssertividadeChart: histÛrico n„o È array, usando array vazio');
            historico = [];
        }
        ...
        try {
            ...
            // Cores fixas para consistÍncia (definidas em updateColors, mas reforÁadas aqui)
            ...
            if (total === 0) {
                ...
                chartInstance.data.datasets[0].borderWidth = 0; // Borda padr„o È 0 ou controlada pelo tema
    ...
    updatePatrimonioChart(historico, capitalInicial, chartInstance, isGlobal = false) {
        try {
            logger.debug('üìä ATUALIZANDO GR√FICO DE PATRIM√îNIO:', {
            ...
            if (!Array.isArray(historico)) {
                logger.warn('‚öÔ∏ Hist√≥rico n√£o √© array:', { type: typeof historico });
                historico = [];
            }

            if (typeof capitalInicial !== 'number' || isNaN(capitalInicial)) {
                logger.warn('‚öÔ∏ Capital inicial inv√°lido:', { capitalInicial });
                capitalInicial = 0;
            }
                ...
                if (op && typeof op.valor === 'number' && !isNaN(op.valor)) {
                    ...
                    if (
                        ...
                        if (op.isWin !== undefined) {
                            ...
                            val = 0;
                            logger.debug(
                                `‚öÔ∏ Opera√ß√£o ${index} com formato n√£o reconhecido, usando valor 0:`,
                                { op }
                            );
                        } else {
                            logger.warn(`‚öÔ∏ Opera√ß√£o ${index} inv√°lida:`, { op });
                            return; // Skip esta opera√ß√£o
                        }
                        runningCapital += val;
                        capitalHistory.push(runningCapital);
                    } else {
            ...
            logger.debug('‚úÖ Dados do gr√°fico atualizados:', {
            ...
            logger.error('‚å updatePatrimonioChart: erro ao atualizar dados', {
                error: String(error),
            });
            return false;
        }
    ...
    /**
     * Atualiza os charts da modal de Replay com dados de uma sess√£o espec√fica
     */
    updateReplayCharts(sessao) {
        try {
            ...
            if (this.replayPatrimonioChart) {
                // Propagar tamb√©m para o gr√°fico de patrim√¥nio (plugin global usa as mesmas stats)
    ...
    /**
     * Atualiza gr√°ficos globais com dados agregados
     */
    updateGlobal(aggregatedData) {
        try {
            if (!aggregatedData) {
                logger.warn('‚öÔ∏ updateGlobal: dados agregados n√£o fornecidos');
                return false;
            }
            ...
            if (!Array.isArray(historico)) {
                logger.warn('‚öÔ∏ updateGlobal: hist√≥rico n√£o √© array, usando array vazio');
                historico = [];
            }

            if (typeof capitalInicial !== 'number' || isNaN(capitalInicial)) {
                logger.warn('‚öÔ∏ updateGlobal: capital inicial inv√°lido, usando 0');
                capitalInicial = 0;
            }

            // Calcula stats para texto central uma √∫nica vez
            const wins = historico.filter((op) => op && (op.resultado === 'win' || op.isWin === true)).length;
            const totalOps = historico.length;
            const winRatePct = totalOps > 0 ? (wins / totalOps) * 100 : 0;

            // Atualiza gr√°ficos da DASHBOARD se existirem
            if (this.dashboardAssertividadeChart) {
                // Propaga stats ao plugin de texto central (usado tamb√©m em Replay)
                this.dashboardAssertividadeChart.$currentStats = {
                    winRate: winRatePct,
                    totalOperations: totalOps,
                };

                // Normaliza hist√≥rico para fun√ß√£o de atualiza√ß√£o
            ...
            if (this.dashboardPatrimonioChart) {
                // Tamb√©m propaga stats para o gr√°fico de patrim√¥nio, pois o plugin √© global
            ...
            logger.debug('‚úÖ updateGlobal: gr√°ficos globais atualizados', {
            ...
            logger.error('‚å updateGlobal: erro ao atualizar gr√°ficos globais', {
                error: String(error),
            });
            return false;
        }
    ...
    updateColors() {
        ...
        // ?? Cores fixas do Progresso das Metas (consistÍncia visual)
        const winColor = '#00d9a6';  // Verde para vitÛrias
        ...
        // Atualiza gr·ficos de PatrimÙnio (Line) com cor verde
        [this.dashboardPatrimonioChart, this.replayPatrimonioChart].forEach((chart) => {
            if (chart) {
                chart.data.datasets[0].borderColor = winColor;
                chart.data.datasets[0].backgroundColor = 'rgba(0, 217, 166, 0.1)'; // Verde com transparÍncia
    ...
    async _initPerformanceOptimizations() {
        try {
            ...
            // Configurar limpeza autom√°tica de performance
            this._setupPerformanceCleanup();

            this._performanceOptimized = true;
            console.log('‚úÖ Otimiza√ß√µes de performance inicializadas');
        } catch (error) {
            console.error('‚å Erro ao inicializar otimiza√ß√µes:', error);
            this._performanceOptimized = false;
        }
    },

    /**
     * Configura limpeza autom√°tica de dados de performance
     */
            ...
            () => {
                ...
                if (window.smartDebouncer) {
                    ...
                    if (stats.pendingCount > 50) {
                        console.warn('üßπ Muitos updates pendentes, limpando...');
    ...
    /**
     * Obt√©m relat√≥rio de performance dos charts
     */
